...
import { ACHIEVEMENT_META, AXES, BUCKET_META, PRESETS } from "@/lib/honor";
import type { AchievementType, Bucket } from "@/lib/types";
...
export default function MethodologyPage() {
  const { t } = useI18n();
  const rows = (Object.entries(ACHIEVEMENT_META) as [AchievementType, (typeof ACHIEVEMENT_META)[AchievementType]][])
    ...
    <div className="mx-auto max-w-3xl px-5 py-12">
            ...
            <tbody>
              {rows.map(([type, meta]) => (
                <tr key={type} className="border-b border-border/60 last:border-0">
                  <td className="px-5 py-2.5 text-sm text-fg">{t(`honorType.${type}`)}</td>
        ...
        {PRESETS.map((p) => (
          <Card key={p.key}>
            ...
            <CardContent className="space-y-2">
              {(Object.keys(p.weights) as Bucket[]).map((b) => (
      ...
      <Card className="mt-4">
        <CardContent className="space-y-3 pt-5">
          {AXES.map((axis) => (
            <div key={axis} className="flex gap-3">
              <span className="w-24 shrink-0 text-sm font-medium text-fg">{t(`axis.${axis}`)}</span>
              <span className="text-sm text-fg-muted">{t(`axisDesc.${axis}`)}</span>
            </div>
          ))}
        </CardContent>
      </Card>
